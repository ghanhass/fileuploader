 ...
 * @param {Bool} config.isMultiple - true/false whether one or multiple file inputs are treated 
 */
function hfu(config) {
   var texts = {
       "en": {
   ...
   if (config.hfuFilesListClass) { // User custom class for hfuDropHere
   ...
   hfuFileinputPrototype.addEventListener("change", function() {
       ...
       console.log(this.files);
		  ...
		  newDivElement.innerHTML = "<span style='margin-right:5px'>"+this.files[i].name+"</span><button type='button' class='hfiRemoveFileBtn'>X</button>";
